...
class Paths(object):
    ...
    @staticmethod       
    def find_path(topology_graph, source_set, destination_set, source):
...
#        destination_temp = destination_set.difference(destination_already_found)
        ...
        for destination in destination_set:
            ...
            if source == destination:
                return
            if source < destination and source in destination_set and destination in source_set:
                return
            queue = [[{source: 1}]]
            path_found = False
            while queue:
    ...
    def path(self):
        ...
        function = partial(self.find_path, self.topology_graph, source_set, destination_set)

        self.pool.map(function, source_set)
        self.pool.close()
