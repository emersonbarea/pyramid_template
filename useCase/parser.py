...
class Parser(object):
    ...
    def time_interval(self):
        try:
            start_datetime = int(datetime.strptime(
                str(self.data_from['data']['query_starttime']).replace('T', ' '),
                '%Y-%m-%d %H:%M:%S').strftime('%s'))
            end_datetime = int(datetime.strptime(
    ...
    def network_by_autonomous_system(self, time_interval, slots, networks):
...
#            print('\n-------------------------\n')
            ...
            #print(df_networks_announced_by_autonomous_systems.loc['147.65.0.0/16', 3333])
        ...
        try:
            ...
            df_prefix_by_as_number = pd.DataFrame(data=prefix_by_as_number, columns=['id_as_number', 'network', 'autonomous_system'])
            df_prefix_by_as_number = df_prefix_by_as_number.drop_duplicates(subset=['id_as_number', 'network'], keep='last')

            df_prefix_by_as_number = df_prefix_by_as_number.groupby(['network', 'autonomous_system']).count()['id_as_number'].reset_index()
            df_prefix_by_as_number.columns = ['network', 'autonomous_system', 'slot-0']

            print('\n-------------------------\n')
            print('df_prefix_by_as_number')
            print(df_prefix_by_as_number)
            print('\n-------------------------\n')

            for index, row in df_prefix_by_as_number.iterrows():
                #print(row['network'], row['autonomous_system'], row['slot-0'])
                registry = df_networks_announced_by_autonomous_systems.loc[row['network'], row['autonomous_system']]
                registry['slot-0'] = registry['slot-0'] + row['slot-0']
...
def main(argv=sys.argv[1:]):
    try:
        ...
        time_interval = parser.time_interval()
        ...
        #
        parser.network_by_autonomous_system(time_interval, slots, networks)
    except:
